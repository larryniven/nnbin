...
void learning_env::run()
{
	...
	std::vector<std::string> labels = speech::load_label_batch(label_batch.at(nsample));
        ...
        if (nsample >= frame_batch.pos.size()) {
            break;
        }
        ...
        for (int i = 0; i < layer; ++i) {
            if (ebt::in(std::string("dropout"), args)) {
                trans.layers.push_back(std::make_shared<cnn::dropout_transcriber>(
        ...
        feat = autodiff::emul(feat, autodiff::dropout_mask(feat, dropout, gen));
